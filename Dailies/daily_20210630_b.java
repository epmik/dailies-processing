...
public class daily_20210630_b extends AbstractDaily 
{
  ...
  private int _minRadius = (int)(ScreenWidth * SketchResolutionMultiplier * 0.35);
  private int _maxRadius = (int) (ScreenWidth * SketchResolutionMultiplier * 0.80);
  
  // private PImage _skinColorPick;
  // private int _skinColorRow;

  public daily_20210630_b() 
  {
    super();
  ...
  private void DrawTriangle(PGraphics canvas) 
  {
    var radius = RandomGenerator.Value(_minRadius, _maxRadius);
    ...
    for (var i = 0; i < points.length; i++) 
    {
      ...
      points[i] = new Point3D((canvas.width / 2) + Math.Cos(angle) * radius,
          (canvas.height / 2) + Math.Sin(angle) * radius, 1);
    }
