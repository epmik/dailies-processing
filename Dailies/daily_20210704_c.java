...
public class daily_20210704_c extends AbstractDaily 
{
  ...
  public void setup() 
  {
    ...
    _colorRandomGenerator = new RandomGenerator();
  ...
  public void Draw(double elapsedTime) 
  {
    ...
    //Canvas.rotate((float)MathUtility.ToRadians(45));

    // Canvas.fill(0);

    // Canvas.rect(0, 0, 100, 100);

    DrawLeaf(_backgroundNoiseGenerator, Canvas);
    ...
    for (var y = 0; y < canvas.height; y++)
    {
      for (var x = 0; x < canvas.width; x++)
      {
        ...
        if(n > -factor && n < factor)
        {
          ...
          canvas.pixels[index] = Color.HsbToInt(
              Color.HueAdjust(hsba[0], 10, 10),
              Color.SaturationAdjust(hsba[1], 0.075, 0.075), 
              Color.BrightnessAdjust(hsba[2], 0.075, 0.075), 
              hsba[3]);
        }
      }
    }
  ...
  private void DrawLeaf(INoiseGenerator noiseGenerator, PGraphics canvas)
  {
    var height = 100.0;
    var middle = 75.0;
    var halfwidth = height * 0.35 * 0.5;
    ...
    for (var h = 0.0F; h < height; h++)
    {
      ...
      f = halfwidth * Easing.InOutCubic(f);

      canvas.vertex(-(float)f, h);
      canvas.vertex((float)f, h);
    }
  ...
  private void DrawColorField(ColorField colorField, PGraphics canvas)
  {
