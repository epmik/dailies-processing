...
public class daily_20210707_c extends AbstractDaily 
{
  ...
  private float _minLineOffset = 120.0f;
  private float _maxLineOffset = 800.0f;

  private int _numberOfDots = 400;
  private float _minDotWeight = 1.0f;
  private float _maxDotWeight = 16.0f;
  private float _minDotOffset = 120.0f;
  private float _maxDotOffset = 800.0f;
  ...
  private float _maxTriangleRatio = 0.24f;
  private float _minTriangleOffset = 120.0f;
  private float _maxTriangleOffset = 800.0f;

  private CallbackListener _resetCallbackListener;

  private ArrayList<IExplode> _explodeArrayList = new ArrayList<IExplode>();

  public daily_20210707_c() 
  {
    super();
  ...
  private void SetupRandomExplodeObjects()
  {
    ...
    for (var i = 0; i < _numberOfDots; i++)
    {
      ...
      dot.Color(Color.HsbToInt(Color.DegreesToDouble(RandomGenerator.Value(40, 60)), RandomGenerator.Value(0, 0.02),
          RandomGenerator.Value(0.98, 1.0)));
    ...
    {
      var triangle = RandomTriangle(_minTriangleWidth, _maxTriangleWidth, _minTriangleRatio, _maxTriangleRatio, _minTriangleOffset, _maxTriangleOffset, RandomGenerator);
  ...
  {
    return RandomDot((float)randomGenerator.Value(minweight, maxweight), (float)randomGenerator.Value(minoffset, maxoffset),
        randomGenerator);
  }
