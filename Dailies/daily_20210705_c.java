...
public class daily_20210705_c extends AbstractDaily 
{
  ...
  private boolean _enableModulate = true;
  ...
  public void setup() 
  {
    ...
    Gui.addToggle("_enableOverlay").setPosition(guiX, guiY).setLabel("Enable Overlay").setValue(_enableOverlay);

    guiY += guiOffsetY;

    Gui.addToggle("_randomLeafAngle").setPosition(guiX, guiY).setLabel("Random Leaf Angle").setValue(_randomLeafAngle);

    guiY += guiOffsetY;

    Gui.addToggle("_enableAttractor").setPosition(guiX, guiY).setLabel("Enable Attractor").setValue(_enableAttractor);

    guiY += guiOffsetY;

    // Gui.addColorPicker("_overlayColor").setPosition(guiX, guiY).setLabel("Overlay Color").setColorValue(_overlayColor);
  ...
  public void Draw(double elapsedTime) 
  {
    ...
    for (var l = 0; l < 400; l++)
    {
      ...
      Canvas.translate((float) (xcenter + x), (float) (ycenter + y));

      if (_randomLeafAngle)
      {
        a = RandomGenerator.Value(0, Math.Pi * 2.0);
      }
